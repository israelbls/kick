...
**`/kick-pull` flags:** `--dry-run` (preview + verdict, transfers nothing), `--code-only` / `--convo-only` (partial), `--keep-remote` (don't stop the remote), `--fork` (land the cloud session under a new id + branch instead of overwriting), `--name "<title>"` (name the pulled session — defaults to `<project> (pulled <date time>)` — so it's easy to find in the `/resume` picker). On true divergence (you advanced locally *and* on the remote), pull asks you — `remote-wins` / `fork` / `abort` — and always backs up local work first (`refs/kick/pre-pull-*`, a git stash, and a timestamped transcript copy).
